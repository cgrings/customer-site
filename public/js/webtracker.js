...
pageView = function (clientId, clientKey, pageUrl) {
  console.log('test2: ' + $.extend(true, {}));
  if(typeof(pageUrl) !== "string") {
    pageUrl = window.location.href;
  }
  console.log('pageUrl: ' + pageUrl);
  var tuid = trackerUid();
  var dataJSON = JSON.stringify({id: tuid, url: pageUrl, dtz: new Date().toISOString()});
  console.log('data: ' + dataJSON);
  var request = $.ajax({
    url: "https://hit-receiver.herokuapp.com/api/page/hit",
    method: "post",
    dataType: 'json',
...
$(function() {
  $( "form" ).submit(function( event ) {
    ...
    if(dataJSON['mail'] !== '') {
      ...
      console.log('data: ' + dataJSON);
      var request = $.ajax({
        url: "https://hit-receiver.herokuapp.com/api/contact",
        method: "post",
        dataType: 'json',
        contentType: "application/json; charset=utf-8",
        data: dataJSON
